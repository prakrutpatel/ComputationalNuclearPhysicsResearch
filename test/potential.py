...
vol_ratio = True
integral_approx = True
random_beta_bool = False
...
def random_beta(betas):
    random_sign = random.randrange(0, 2)
    if random_sign == 0:
        new_beta = betas + (0.5*random.random()*betas)
    if random_sign == 1:
        new_beta = betas - (0.5*random.random()*betas)
    if new_beta > 0.9:
        new_bata = 0.9
...
if random_beta_bool:
   for i in range(len(beta)):
       beta[i] = random_beta(beta[i])
   print("RANDOM Beta:",beta)                    
#*********************************
...
scale = 0.5  # Scale of modification (% of formula to apply) for real
delta_r = 1.0
...
def vol_int_v(vp):
    if integral_approx == False:
        total = tplquad(diff_vol1, phi1, phi2, lambda y: theta1, lambda y: theta2, lambda y, z: r1, lambda y, z: r2)
        return total
    else:
        yv = (np.pi*A_v/R_v)
        total = (4/3)*np.pi*((R_v ** 3)/atomic_number)*Vv*(1+(yv ** 2))
        return [total]


def vol_int_v_m(vp):  # volume term integral modified
    if integral_approx == False:
        total = tplquad(diff_vol1_m, phi1, phi2, lambda y: theta1, lambda y: theta2, lambda y, z: r1, lambda y, z: r2)
        return total
    else:
        yv = (np.pi*A_v_m/R_v_m)
        total = (4/3)*np.pi*((R_v_m ** 3)/atomic_number)*Vv*(1+(yv ** 2))
        return [total]

def vol_int_v_m2(vp):  # volume term integral modified
    if integral_approx == False:
        total = tplquad(diff_vol1_m2, phi1, phi2, lambda y: theta1, lambda y: theta2, lambda y, z: r1, lambda y, z: r2)
        return total
    else:
        yv = (np.pi*A_v_m/R_v_m2)
        total = (4/3)*np.pi*((R_v_m2 ** 3)/atomic_number)*Vv*(1+(yv ** 2))
        return [total]
...
vol_ratio_v2 = vol_int_v(vp)[0] / vol_int_v_m2(vp)[0]
...
print(Element,E,vol_ratio_v, file=open('vol_ratio.txt','a'))
print(Element,E,vol_ratio_v2, file=open('vol_ratio.txt','a'))
...
if case == 'off':
    ...
    for i in range(0,len(angles)):
        print("{:9.1f}{:11.4E}{:5.2f}".format(angles[i],cs[i],(3.00+((i/(len(angles)-1))*22))),
