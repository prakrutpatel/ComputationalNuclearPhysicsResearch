...
def import_data(filename, word, array):
    ...
    array.append(np.loadtxt(filename, skiprows=loc[0], max_rows=169, usecols=(3), converters =conv(1)))
    for i in range(1, len(loc)):    
        array.append(np.loadtxt(filename, skiprows=loc[i], max_rows=169, usecols=(2), converters =conv(1)))
    return array
...
for i in range(0, len(datapath)):
    ...
    for j in range(0, len(files[i])):
        ...
        import_data(files[i][j], 'elastic scattering', asym)
asym = np.array_split(asym, len(energy))
for i in range(0, len(asym)):
    asym[i] = np.array_split(asym[i], len(order))
...
for i in range(0, len(energy)):
    for j in range(0, len(order)):
        for k in range(0, len(states)):  
            axs[k].plot(angle, 8+(-2*i)+asym[i][j][k], color=colors[j], linestyle=linestyles[j])
